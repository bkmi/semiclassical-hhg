...
# Milosevic, et al. for two color elliptical field mixing. The parameters of the envelope as well
...
#recombTimes = bounds for program to consider HHG, this program considers a 2*(longest period) for recomb (tf)

recombTimes = [0.0, 2]  #given in field cycles

if period1 > period2:
    recomb_grid = np.array(np.linspace( (recombTimes[0]) * period1, (recombTimes[1])
                                       * period1, recombination_grid_size) )

    #The tau guesses are limited to 1 period or less because anything longer gets removed later anyway.
    #Generate real tau grid
    real_tau_grid = np.array(np.linspace( 0.0, period1, real_tau_grid_size), dtype = np.complex128 )
    #Generate imag tau grid
    imag_tau_grid = np.array(np.linspace( 0.0, period1, imag_tau_grid_size), dtype = np.complex128 )
else:
    recomb_grid = np.array(np.linspace( (recombTimes[0]) * period2, (recombTimes[1])
                                       * period2, recombination_grid_size) )

    #The tau guesses are limited to 1 period or less because anything longer gets removed later anyway.
    #Generate real tau grid
    real_tau_grid = np.array(np.linspace( 0.0, period2, real_tau_grid_size), dtype = np.complex128 )
    #Generate imag tau grid
    imag_tau_grid = np.array(np.linspace( 0.0, period2, imag_tau_grid_size), dtype = np.complex128 )
...
# Name the output data. Files will be placed in a directory: data_cw_ellip_(filename)
...
# The exception is raised if the directory ./data_cw_ellip_(filename) already exists, no matter what is inside.
...
try:
    os.mkdir("./data_cw_ellip_"+str(filename))
except OSError:
    print("The directory " + "./data_cw_ellip_" + str(filename) + " already exists. \n")
    if query_yes_no("Would you like to continue and overwrite the data in the directory?"):
        print("\nOkay, I will overwrite the data in " + "./data_cw_ellip_" + str(filename))
        pass
    else:
        print("\nYou have chosen to stop the program. \nRename the variable 'filename' such that "
              + "./data_cw_ellip_(filename) does not already exist. \nThe filename is currently: "
              + str(filename) + "\nThe directory that already exists is called: " + "./data_cw_ellip_"
              + str(filename))
        raise
...
from semiclassicalhhg.cw_ellip_supp_eqns import nineteen_cplx_tf, nineteen_cplx_tf_p, position, velocity
...
def efieldvec( t, args ):
    ...
    xdir = (((1.0j/2)*EF1)/(e**(1.0j*int1*t*w)*sqrt(1 + epsilon1**2))
            - ((1.0j/2)*e**(1.0j*int1*t*w)*EF1)/sqrt(1 + epsilon1**2)
            + ((1.0j/2)*EF2)/(e**(1.0j*int2*t*w)*sqrt(1 + epsilon2**2))
            - ((1.0j/2)*e**(1.0j*int2*t*w)*EF2)/sqrt(1 + epsilon2**2))  
    
    ydir = (-(EF1*epsilon1)/(2*e**(1.0j*int1*t*w)*sqrt(1 + epsilon1**2))
            - (e**(1.0j*int1*t*w)*EF1*epsilon1)/ (2*sqrt(1 + epsilon1**2))
            - (EF2*epsilon2)/(2*e**(1.0j*int2*t*w)*sqrt(1 + epsilon2**2))
            - (e**(1.0j*int2*t*w)*EF2*epsilon2)/(2*sqrt(1 + epsilon2**2)))
...
with open('./'+"data_cw_ellip_"+str(filename)+'/information', 'w') as info:
    info.write(what_to_say)


# In[ ]:

np.save('./'+"data_cw_ellip_"+str(filename)+'/params', np.array(params))


# In[ ]:

np.save('./'+"data_cw_ellip_"+str(filename)+'/intensity_coef', np.array(intensity_coef))


# In[ ]:

np.save('./'+"data_cw_ellip_"+str(filename)+'/saddle_pts', np.array(quick_pos_uniq_round_vals))
...
try:
    os.mkdir("./data_cw_ellip_"+str(filename)+"/data_by_intensity_ratio")
except:
    pass

for i, v in enumerate(params):
    if i<9:
        np.savetxt('./'+"data_cw_ellip_"+str(filename)+"/data_by_intensity_ratio/0"+str(i)+"_params",
                  np.array(params)[i])
        np.savetxt('./'+"data_cw_ellip_"+str(filename)+"/data_by_intensity_ratio/0"+str(i)+"_intensity_coef",
                  np.array(intensity_coef)[i])
        np.savetxt('./'+"data_cw_ellip_"+str(filename)+"/data_by_intensity_ratio/0"+str(i)+"_saddle_pts",
                  np.array(quick_pos_uniq_round_vals)[i])
    else:
        np.savetxt('./'+"data_cw_ellip_"+str(filename)+"/data_by_intensity_ratio/"+str(i)+"_params",
                  np.array(params)[i])
        np.savetxt('./'+"data_cw_ellip_"+str(filename)+"/data_by_intensity_ratio/"+str(i)+"_intensity_coef",
                  np.array(intensity_coef)[i])
        np.savetxt('./'+"data_cw_ellip_"+str(filename)+"/data_by_intensity_ratio/"+str(i)+"_saddle_pts",
                  np.array(quick_pos_uniq_round_vals)[i])
